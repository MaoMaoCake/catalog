...
## What's included in this Cloudnative PG Plugin Barman Cloud image

This Docker Hardened Cloudnative PG Plugin Barman Cloud image contains the barman cloud plugin for CloudNativePG that
...
## Installing Cloudnative PG Plugin Barman Cloud using the latest manifest

Download the lastest manifest for the plugin at
https://github.com/cloudnative-pg/plugin-barman-cloud/releases/download/<version>/manifest.yaml, replacing <version>
with the version being installed. In the resulting yaml file replace the values in the image section to use the DHI
image

```yaml
image:
	registry: docker.io,
	repository: dhi.io/cloudnative-pg-plugin-barman-cloud,
	tag: <tag>,
```

Apply the manifest to install the plugin to kubernetes
...
```
kubectl rollout status deployment -n <namespace> barman-cloud
```

## Installing Cloudnative PG Plugin Barman Cloud using Helm

Alternatively, you can install Cloudnative PG Plugin Barman Cloud using the official helm chart and replace the image.
Replace `<your-registry-secret>` with your [Kubernetes image pull secret](https://docs.docker.com/dhi/how-to/k8s/) and
`<tag>` with the desired image tag.
...
helm install barman-cloud cloudnative-pg/plugin-barman-cloud \
  ...
  --set image.repository=plugin-barman-cloud \
  --set image.tag=<tag>  
```
...
```
kubectl rollout status deployment -n cnpg barman-cloud
```
